...
1. Ask exactly: **“Whose recipes are you adding it to?”** Offer every person returned by the live owner lookup, followed by “Add somebody else.” The returned list is authoritative and includes people added by earlier skill runs; preserve the returned owner ID when one is selected. If the owner lookup was unavailable, offer Sammy, Autumn, Addison, Sam G, or “Add somebody else.” If they choose somebody else, ask for that person’s display name, derive a lowercase hyphenated ID and initials, and mark the owner as new for the preview.
...
The script reads the connection from environment variables or the local credentials file created by `node scripts/configure.mjs`. If no connection is configured, ask the contributor to paste the 30-day add-only guest code from the Recipe Book website's **Add** panel into the chat. Explain that this guest code can add recipes only and will remain in their chat history until it expires. After they provide it, use it once to run the bundled setup, do not repeat or print it, and save it only in the local credentials file. Interactive Terminal setup, `CARUSO_RECIPE_ADD_TOKEN`, `--code-stdin`, and `--code-file` remain available as alternatives. Never request, accept, store, or use a GitHub token, Vercel token, repository write credential, or general deployment credential.

The service only appends a new recipe and, when “Add somebody else” was chosen, atomically appends that person’s permanent recipe-book profile. The first recipe creates a new subsection/tab using the person’s name and initials. Future owner lookups return that saved person as a normal choice, so never fall back to the original four names when lookup succeeds. It rejects duplicate IDs, unsupported fields, malformed data, source-format drift, and non-fast-forward updates. The resulting Git commit starts the normal production deployment.
