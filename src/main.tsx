  ...
  return (
    <>
      ...
      <main id="main-content" className={`${interiorPageStyles.page} dedicated-page products-page`} tabIndex={-1}>
        ...
        <section className="products-operating" aria-labelledby="products-operating-title"><div className={`${interiorPageStyles.sectionShell} products-shell`}><header data-reveal><h2 id="products-operating-title">Operating products</h2></header><div className="products-operating-grid" data-stagger>{operatingProducts.map((product) => <Link href={product.href} key={product.id}><span className="products-icon">{product.icon}</span><div><strong>{product.name}</strong><p>{product.problem}</p></div><ArrowRight size={17} aria-hidden="true" /></Link>)}</div></div></section>
